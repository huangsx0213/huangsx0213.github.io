...
下面给出一份完整的说明书，对项目的各个模块、目录结构、程序逻辑、以及 Excel 用例填写（API 和 Web）的各个细节进行了详细描述。由于篇幅较大，本文分为多部分进行说明，以下为第一部分，后续部分可以继续补充。
...
接下来的章节分别说明 API 测试与 Web UI 测试的程序逻辑以及 Excel 用例填写要求。以下内容基于本框架的实现说明，便于测试工程师了解整个流程与各项字段如何定义。
...
下面给出完整说明书的第二部分，详细描述 Web UI 测试模块的程序逻辑以及 Excel 测试用例文件的填写规则与注意事项。

---

## 第二部分：Web UI 测试模块详细说明
